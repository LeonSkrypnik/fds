...
pub fn derive_kek_argon2id(password: &str, salt: &[u8; 16], m_cost_kib: u32, t_cost: u32) -> anyhow::Result<[u8; KEY_LEN]> {
    ...
    let hash = argon2.hash_password_customized(password.as_bytes(), None, None, params, &salt_string)?;
    ...
    let hk = Hkdf::<Sha256>::new(None, hash.hash.ok_or_else(|| anyhow::anyhow!("argon2 missing hash"))?.as_bytes());
    hk.expand(b"vault-kek", &mut out)?;
    Ok(out)
}

pub fn aead_encrypt(key: &[u8; KEY_LEN], nonce12: &[u8; 12], aad: &[u8], plaintext: &[u8]) -> anyhow::Result<Vec<u8>> {
    let cipher = ChaCha20Poly1305::new(Key::from_slice(key));
    let nonce = Nonce::from_slice(nonce12);
    let mut out = cipher.encrypt(nonce, chacha20poly1305::aead::Payload { msg: plaintext, aad })?;
    Ok(out)
}

pub fn aead_decrypt(key: &[u8; KEY_LEN], nonce12: &[u8; 12], aad: &[u8], ciphertext: &[u8]) -> anyhow::Result<Vec<u8>> {
    let cipher = ChaCha20Poly1305::new(Key::from_slice(key));
    let nonce = Nonce::from_slice(nonce12);
